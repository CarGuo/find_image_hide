...
### 2026-06-03（P2.4 code-review 修复 · 4 条 nice-to-have）

- **double-skill 交叉验证**：[TRAE-code-review](file:///d:/workspace/project/find_image_hide/.trae/skills) §5.5 双 sub-agent 交叉验证 + fallback 自查模式开 7 条候选 → drop 3 条 false positive（noqa 多余 / `_resolve_per_image_dir` traversal 是 pre-existing § 0 排除 / innerHTML 注入但变量已 escapeHtml）→ 保留 4 条 nice-to-have；[TRAE-security-review](file:///d:/workspace/project/find_image_hide/.trae/skills) Pass A 基线 + Pass B 偏离图 + Pass C 源-汇追踪 5 个候选攻击面 confidence < 0.7 全 drop → **No exploitable issues found**
- **Issue #1 — 流式 SHA**：[render_pdf](file:///d:/workspace/project/find_image_hide/image_forensics/pdf_report.py#L334-L352) 原本 `read_bytes() + _sha256_bytes(raw) + json.loads(raw.decode())` 三份内存拷贝 ⇒ 改 `_sha256_file(report_path)` 流式 4KB chunked + `read_text(encoding="utf-8")`，避免大 report.json 同时驻留 raw bytes / utf-8 解码副本 / json 对象三份（千张图批量场景下显著降内存峰值）
- **Issue #2 — 复合 hash 唯一性**：原 `source_report_sha256` 仅来自 report.json，但 [render_pdf](file:///d:/workspace/project/find_image_hide/image_forensics/pdf_report.py#L334-L352) 接受可选 `image_path` 参数把图片真实 sha 写回 report 后再渲染 ⇒ 同一份 report.json + 不同 image_path 会产同 hash（伪造容易）。修法：manifest 新增 `image_path_sha256`（image bytes hash）+ `source_inputs_sha256 = sha256(f"{report_sha}:{image_sha}")` ASCII 编码复合 hash；chain-of-custody 凭据页动态加 2 行；webapp 仅在调用方真实传 image_path 时才输出 `X-Image-Sha256` / `X-Source-Inputs-Sha256` 头（生产 webapp 路由不传，所以这两头缺失，符合"条件头"语义）
- **Issue #3 — viz_count 静默降级可见化**：原 [_build_visualization_pages](file:///d:/workspace/project/find_image_hide/image_forensics/pdf_report.py#L285-L315) 只返回 flow，调用方无从知道实际嵌入了几张图证（缺图静默 fallback，律师不知道证据完整性是否打折）⇒ 返回签名升级 `tuple[list[Any], int]`；manifest 加 `viz_count`；webapp 新增 `X-Pdf-Viz-Count` 头；chain_table 加"图证嵌入数 N / 6"行；前端 [bindPdfExport](file:///d:/workspace/project/find_image_hide/webui/static/image.js#L818-L860) 状态行显示"含 N 张图证"
- **Issue #4 — 探测异常未短路**：原 `bindPdfExport` 探测 `/api/pdf/status` 失败时只更新文案不禁用按钮 ⇒ 用户仍可点击触发 501 噪声 ⇒ catch 分支与 `!st.available` 分支对齐 `btn.disabled = true; btn.title = ...; if (statusEl) ...; return;`
- **回归证据**：四路并行（A 路 run_regression / B 路 PDF 跨样本可重现深验 12/12 / C 路 code-review / D 路 security-review）+ 修复后双轮验证 — 模块单测 13/13（原 6 项 + 新 a image_path_sha256 / b source_inputs_sha256 / c viz_count==N when viz / viz_count==0 when no viz / d image_path 路径下二次渲染字节级一致）+ webapp HTTP E2E 15/15（**关键 B5/B6 验证 `X-Image-Sha256` / `X-Source-Inputs-Sha256` 在 webapp 不传 image_path 路径下正确缺失**，B4 验证 `X-Pdf-Viz-Count=6`，B11 二次下载 sha 一致）；diagnostics 全空
- **遗留 P2.5 backlog（非 P2.4 引入）**：`tools/run_regression.py` 在 commit `dd9cc4d`（P2.3）和 `7aa6aa8`（P2.4）上跑均产 `normal_avif=MEDIUM / normal_png=UNKNOWN`，与会话早期记忆"avif=LOW / png=HIGH"不一致 — `git stash` 切回 P2.3 复跑确认是 pre-existing scoring/启发式漂移（FFT 异常对称峰 + Copy-Move SNR + ai_heuristics 在 1419 byte 超低分辨率合成 AVIF 的边界行为），与本轮 PDF 改动零因果。归档独立 issue 不挡 P2.4 修复
- **跨平台 + 零新依赖**：Python 2 处（pdf_report.py / webapp.py）+ JS 1 处（image.js），纯 stdlib + reportlab + Flask；Win/macOS/Linux × Py 3.9-3.13 全 PyPI 覆盖
